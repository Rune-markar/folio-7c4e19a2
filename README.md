...
歴史地図の選択順は「01 年代 → 02 地域 → 03 国名」とする。01と02は同じ行のコンパクトな選択欄とし、押した時だけ縦一覧の選択ウィンドウを開く。03のカード見出しと選択後の地図見出しは重複表示を避けて国名だけを示し、通貨名は詳細内の「当時の通貨」へ集約する。同じ年代・国名に複数の通貨期がある場合は内部では別々の選択肢として扱い、カードとアクセシブルネームの年代で区別する。地域の正本は台帳の `collectionIndex` とし、収蔵品の `region` は一致させる。歴史上の国号と台帳名が一致しない場合だけ `atlasRegionOverrides` で補う。02を選んだ段階では `atlasRegionViews` の概略範囲へ地図を拡大して網掛けし、03を選ぶと対象の国・通貨期の境界表示へ切り替える。地図は1本指またはマウスドラッグで移動し、2本指ピンチ、マウスホイール、画面上のボタンで拡大・縮小できる。リセット操作で現在の選択対象へ戻り、別の年代・地域・国を選ぶと新しい対象へ自動的に再フォーカスする。初回表示は登録済みの最新年代を開き、以後はURLパラメータまたはブラウザ保存値から選択状態を復元する。地図上の選択可能な国を押した場合は、その国について登録済みの最も新しい年代・通貨期へ直接移動する。国選択後に地図右上の「地図の国選択を解除」を押すと、年代と地域を保ったまま地域表示へ戻る。

中央アジア地域表示は、アフガニスタン、アルメニア、アゼルバイジャン、カザフスタン、キルギス、ジョージア、タジキスタン、トルクメニスタン、ウズベキスタンの明示的な集合として扱う。緯度経度の概略範囲がヨーロッパの地域表示と重なっても、これらの国をヨーロッパ側の網掛けへ含めない。
...
変更後は依存パッケージ不要の `node scripts/audit.mjs` と `node scripts/check-atlas-interactions.mjs` を実行する。前者はJSON、ID、地域、カタログ番号、画像出典・権利確認状態、歴史地図境界、通貨系譜、国旗、収蔵スコープ、物語構造を一括確認し、後者は地図見出しとパン・ズーム操作の実装を確認する。
